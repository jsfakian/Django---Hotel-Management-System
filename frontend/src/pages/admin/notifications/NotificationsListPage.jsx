      ...
      <List>
        ...
        {notifications.map((n, i) => (
          <React.Fragment key={n.id}>
            <ListItem alignItems="flex-start" sx={{ bgcolor: n.is_read ? 'transparent' : 'action.hover', cursor: 'pointer', borderRadius: 1 }} onClick={() => !n.is_read && markRead(n.id)}>
              <ListItemAvatar><Avatar sx={{ bgcolor: n.is_read ? 'grey.300' : 'primary.main' }}><Bookmark /></Avatar></ListItemAvatar>
              <ListItemText
                primary={<Typography fontWeight={n.is_read ? 400 : 700}>{n.message || n.title}</Typography>}
